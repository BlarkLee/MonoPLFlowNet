...
def save_checkpoint_e2e(state, state_depth, ckpt_dir, filename='checkpoint.pth.tar', filename_depth='checkpoint_depth.pth.tar'):
    torch.save(state, os.path.join(ckpt_dir, filename))
    #if state['epoch'] % 10 == 1:
    shutil.copyfile(
            os.path.join(ckpt_dir, filename),
    ...
    torch.save(state_depth, os.path.join(ckpt_dir, filename_depth))
    #if state['epoch'] % 10 == 1:
    shutil.copyfile(
            os.path.join(ckpt_dir, filename_depth),
            os.path.join(ckpt_dir, 'checkpoint_depth_'+str(state['epoch'])+'.pth.tar'))
