...
def evaluate(val_loader, model, model_depth, logger, args, gen_lattice):
    ...
    with torch.no_grad():
        for i, items in enumerate(val_loader):
            ...
            depth1_gt_np = torch.squeeze(depth1_gt, 0).detach().cpu().numpy()
            depth2_gt_np = torch.squeeze(depth2_gt, 0).detach().cpu().numpy()
         
            
            image1_max = torch.nn.functional.interpolate(image1, (576, 960))
            image2_max = torch.nn.functional.interpolate(image2, (576, 960))
            img12 = torch.cat([image1_max,image2_max],0)
            for k in range(len(model.module.reg_modules)):
                model.module.reg_modules[k] = flow_reg([1,960//(2**(6-k)), 576//(2**(6-k))], 
            ...
            for k in range(len(model.module.warp_modules)):
                model.module.warp_modules[k] = WarpModule([1,960//(2**(6-k)), 576//(2**(6-k))]).cuda()
            flow2d_max, _, _, _ = model(img12)
            flow2d_est = torch.nn.functional.interpolate(flow2d_max, (540, 960))
            ...
            pc2 = pixel2xyz(depth2_gt_np, P_rect_left, px=px2, py=py2)
            pc1 = pixel2xyz(depth1_gt_np, P_rect_left)
            ...
            ratio_s = 0
            for t in range (8192):
                ratio = (sf_gt[0, 2, t]/output[t, 2]).detach().cpu().numpy()
                output[t, :] = output[t, :] * ratio
