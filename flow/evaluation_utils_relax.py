...
def evaluate_3d(sf_pred, sf_gt):
    ...
    acc3d_strict = (np.logical_or(l2_norm < 0.3, relative_err < 0.1)).astype(np.float).mean()
    acc3d_relax = (np.logical_or(l2_norm < 0.4, relative_err < 0.2)).astype(np.float).mean()
    outlier = (np.logical_or(l2_norm > 0.5, relative_err > 0.3)).astype(np.float).mean()
...
def evaluate_2d(flow_pred, flow_gt):
    ...
    acc2d = (np.logical_or(epe2d < 20., relative_err < 0.2)).astype(np.float).mean()
