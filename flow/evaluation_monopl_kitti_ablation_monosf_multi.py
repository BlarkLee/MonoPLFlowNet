...
def evaluate(val_loader, model, model_depth, logger, args, gen_lattice):
    ...
    with torch.no_grad():
        for i, items in enumerate(val_loader):
            ...
            K_flip = torch.Tensor([[fu, 0, float(image2.shape[2]-cx)], [0, fv, cy], [0, 0, 1]]).view(1,3,3)
            image_stack = torch.stack([image1, image1, image2], dim=1)
            input_dict = {}
            input_dict['aug_size'] = torch.Tensor([[float(image2.shape[2]), float(image2.shape[3])]])
            ...
            if i in sampled_batch_indices:
                ...
                save_idx += 1
            del pc1_est, sf_gt, generated_data, image1, image2, pc_mask, mask1, mask2
