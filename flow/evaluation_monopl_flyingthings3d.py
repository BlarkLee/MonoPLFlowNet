...
def evaluate(val_loader, model, model_depth, logger, args, gen_lattice):
    ...
    with torch.no_grad():
        for i, items in enumerate(val_loader):
            pc1_gt, pc2_gt, sf_gt, path, depth1_gt, depth2_gt, image1, image2, pc_mask, mask1, mask2, generated_data, img1_pth, lattice_time = items
            ...
            if ((generated_data[0]['pc1_hash_cnt']<generated_data[1]['pc1_hash_cnt']) or (generated_data[0]['pc2_hash_cnt']<generated_data[1]['pc2_hash_cnt'])):
                ...
                continue
            if ((generated_data[1]['pc1_hash_cnt']<generated_data[2]['pc1_hash_cnt']) or (generated_data[1]['pc2_hash_cnt']<generated_data[2]['pc2_hash_cnt'])):
                print("forward max cnt low level larger than high level, skip!")
                continue
            ...
            pc2_epe = np.mean(np.sqrt(np.sum((pc2_np - pc2_est_np)**2, axis=2)))
            sf_np = sf_gt.numpy()
            ...
            if i % args.print_freq == 0:
                logger.log('Test: [{0}/{1}]\t'
                           ...
                           'PC2_EPE{pc2_epe_.val:.4f} ({pc2_epe_.avg:.4f})'
            ...
            if i in sampled_batch_indices:
                ...
                save_idx += 1
            del pc1_est, pc2_est, sf_gt, generated_data, depth1_gt, depth2_gt, image1, image2, pc_mask, mask1, mask2
    ...
    print("depth_time", total_time_depth/956)
    print("scene_time", total_time_scene/956)
    print("lattice_time", total_time_lattice/956)
