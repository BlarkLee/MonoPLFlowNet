...
class GenerateDataUnsymmetric(object):
    def __init__(self):
        ...
        self.scales_filter_map = [[3., 1, -1, -1],
                    [2., 1, -1, -1],
                    [1., 1, 1, 1],
                    [0.5, 1, 1, 1],
                    [0.25, 1, 1, 1],
    ...
    def get_keys_and_barycentric(self, pc):
        ...
        #\("rank", rank.shape)
        #print("rank_rang", np.unique(rank.cpu().detach().numpy()))
        #print("greedy", greedy.shape)
        #print("greedy", greedy[:, :, None].shape)
        ...
        #print("barycentric", barycentric.shape)
    ...
    def __call__(self, pc1, pc2, sf):
        ...
        with torch.no_grad():
            ...
            #print("pc1_before", pc1.shape)
            #print("pc2_before", pc2.shape)
            #print("sf_before", sf.shape)

            generated_data_f = []
            #generated_data_b = []
            last_pc1 = pc1.clone().cpu()
            last_pc2 = pc2.clone().cpu()
            ...
            for idx, (scale, bcn_filter_raidus, corr_filter_radius, corr_corr_radius) in enumerate(
                ...
                #print("last_pc1_in_shape", last_pc1.shape)
                #print("last_pc1_in_range", np.unique(last_pc1.detach().cpu().numpy()[2, :]))
                ...
                if idx != len(self.scales_filter_map) - 1:
                    ...
                    #print("last_pc1_out0_shape", last_pc1.shape)
                    #print("last_pc1_out0_range", np.unique(last_pc1.detach().cpu().numpy()[2, :]))
                    ...
                    #print("last_pc1_out1_shape", last_pc1.shape)
                    #print("last_pc1_out1_range", np.unique(last_pc1.detach().cpu().numpy()[2, :]))
                    last_pc2 = torch.matmul(self.elevate_mat.t(), last_pc2)
                    pc1_num_points = pc1_hash_cnt
                    pc2_num_points = pc2_hash_cnt
                '''
                print("index", idx)
                print('pc1_barycentric', pc1_barycentric.shape)
                print('pc2_barycentric', pc2_barycentric.shape)
                print('pc1_el_minus_gr', pc1_el_minus_gr.shape)
                print('pc2_el_minus_gr', pc2_el_minus_gr.shape)
                print('pc1_lattice_offset', pc1_lattice_offset.shape)
                print('pc2_lattice_offset', pc2_lattice_offset.shape)
                print('pc1_blur_neighbors', pc1_blur_neighbors.shape)
                print('pc2_blur_neighbors', pc2_blur_neighbors.shape)
                print('pc1_corr_indices', pc1_corr_indices.shape)
                print('pc2_corr_indices', pc2_corr_indices.shape)
                print('pc1_hash_cnt', pc1_hash_cnt)
                print('pc2_hash_cnt', pc2_hash_cnt)'''
                
                generated_data_f.append({'pc1_barycentric': pc1_barycentric.cuda(),
                                       'pc2_barycentric': pc2_barycentric.cuda(),
                                       'pc1_el_minus_gr': pc1_el_minus_gr.cuda(),
                                       'pc2_el_minus_gr': pc2_el_minus_gr.cuda(),
                                       'pc1_lattice_offset': pc1_lattice_offset.cuda(),
                                       'pc2_lattice_offset': pc2_lattice_offset.cuda(),
                                       'pc1_blur_neighbors': pc1_blur_neighbors.cuda(),
                                       'pc2_blur_neighbors': pc2_blur_neighbors.cuda(),
                                       'pc1_corr_indices': pc1_corr_indices.cuda(),
                                       'pc2_corr_indices': pc2_corr_indices.cuda(),
                                       'pc1_hash_cnt': pc1_hash_cnt,
                                       'pc2_hash_cnt': pc2_hash_cnt,
                                       'pc1_barycentric_3d': last_pc1,
                                       'pc2_barycentric_3d': last_pc2
                                       })
                '''
                generated_data_b.append({'pc1_barycentric': pc2_barycentric.cuda(),
                                       'pc2_barycentric': pc1_barycentric.cuda(),
                                       'pc1_el_minus_gr': pc2_el_minus_gr.cuda(),
                                       'pc2_el_minus_gr': pc1_el_minus_gr.cuda(),
                                       'pc1_lattice_offset': pc2_lattice_offset.cuda(),
                                       'pc2_lattice_offset': pc1_lattice_offset.cuda(),
                                       'pc1_blur_neighbors': pc2_blur_neighbors.cuda(),
                                       'pc2_blur_neighbors': pc1_blur_neighbors.cuda(),
                                       'pc1_corr_indices': pc2_corr_indices.cuda(),
                                       'pc2_corr_indices': pc1_corr_indices.cuda(),
                                       'pc1_hash_cnt': pc2_hash_cnt,
                                       'pc2_hash_cnt': pc1_hash_cnt,
                                       'pc1_barycentric_3d': last_pc2,
                                       'pc2_barycentric_3d': last_pc1
                                       })'''
                
                
            #print("pc1_after", pc1.shape)
            #print("pc2_after", pc2.shape)
            #print("generated data:")
            #print('pc1_hash_cnt', generated_data[0]['pc1_hash_cnt'])
            #print('pc2_hash_cnt', generated_data[0]['pc2_hash_cnt'])
            '''
            for i in range(len(generated_data_f)):
                print("i=", i)
                for k in generated_data_f[i].keys():  
                    if k == 'pc1_hash_cnt' or k == 'pc2_hash_cnt':
                        print(k, ': ', generated_data_f[i][k])
                    else:
                        print(k, ': ', generated_data_f[i][k].shape)'''
            #print(fuck)
            
            return pc1, pc2, sf, generated_data_f#, generated_data_b
...
class ProcessData(object):
    def __init__(self):#, data_process_args, num_points, allow_less_points):
        self.DEPTH_THRESHOLD = 35. #data_process_args['DEPTH_THRESHOLD']
        self.HEIGHT_THRESHOLD = -1.4
        self.no_corr = True #data_process_args['NO_CORR']
        self.num_points = 8192 #*4 #num_points
        self.allow_less_points = True #allow_less_points

    def __call__(self, data, pcl_mask_gt, pc1_gt, pc2_gt):
        pc1, pc2 = data
        pc1_gt = torch.squeeze(pc1_gt).reshape(-1,3).detach().cpu().numpy()
        pc2_gt = torch.squeeze(pc2_gt).reshape(-1,3).detach().cpu().numpy()
        #print("pc1_x", np.unique(pc1_gt[:, 0]))
        #print("pc1_y", np.unique(pc1_gt[:, 1]))
        #print("pc1_z", np.unique(pc1_gt[:, 2]))
        #print(stop)
        if pc1 is None:
            return None, None, None, None, None
        
        sf_gt = pc2_gt[:, :3] - pc1_gt[:, :3]
        #if self.DEPTH_THRESHOLD > 0:
        near_mask_depth = np.logical_and(pc1_gt[:, 2] < self.DEPTH_THRESHOLD, pc2_gt[:, 2] < self.DEPTH_THRESHOLD)
        #near_mask_height_oppo = np.logical_and(pc1_gt[:, 1] < self.HEIGHT_THRESHOLD, pc2_gt[:, 1] < self.HEIGHT_THRESHOLD)
        #near_mask_height = np.logical_not(near_mask_height_oppo)
        #near_mask = np.logical_and(near_mask_depth, near_mask_height)
        near_mask = near_mask_depth
        '''    
        else:
        ...
        if self.num_points > 0:
            try:
                ...
                #print("sampled_indices1", sampled_indices1)
                if self.no_corr:
        ...
        pcl_mask = torch.flatten(torch.squeeze(torch.squeeze(pcl_mask_gt) ,0))
        mask0 = (pcl_mask==1).nonzero().view(-1).detach().cpu().numpy()
        indices = np.arange(len(mask0))
        mask1 = np.sort(np.random.choice(indices, size=self.num_points, replace=False, p=None))
        if self.no_corr:
            mask2 = np.sort(np.random.choice(indices, size=self.num_points, replace=False, p=None))
        else:
            mask2 = mask1
        maskall_1 = mask0[mask1]
        maskall_2 = mask0[mask2]
        #print("sampled_indices2", sampled_indices2)
        #print("pcl_mask_gt", pcl_mask_gt.shape)
        #print("pc1_before", pc1.shape)
        pc1 = pc1[maskall_1]
        #print("sampled_indices1", sampled_indices1.shape)
        #print("maskall_1", maskall_1.shape)
        #print("pc1_after", pc1.shape)
        #print(stop)
        ...
        pc1_gt = pc1_gt[mask1]
        pc2_gt = pc2_gt[mask1]
        return pc1, pc2, maskall_1, maskall_2, sf_gt, pc1_gt, pc2_gt
...
class Augmentation(object):
    def __init__(self): #, aug_together_args, aug_pc2_args, data_process_args, num_points, allow_less_points=False):
        ...
        self.together_args = {'scale_low': 0.95, 'scale_high': 1.05, 'degree_range': 0.1745329252, 'shift_range':1., 'jitter_sigma':0.01, 'jitter_clip':0.00}
        #self.pc2_args = aug_pc2_args
        self.pc2_args = {'degree_range':0., 'shift_range':0.3, 'jitter_sigma': 0.01, 'jitter_clip': 0.00}
        self.DEPTH_THRESHOLD = 35. #data_process_args['DEPTH_THRESHOLD']
        self.HEIGHT_THRESHOLD = -1.4 #0.3
        self.no_corr = True #data_process_args['NO_CORR']
        self.num_points = 8192 #num_points
        self.allow_less_points = False #allow_less_points

    def __call__(self, data, pcl_mask_gt, pc1_gt, pc2_gt):
        pc1, pc2 = data
        pc1_gt = torch.squeeze(pc1_gt).reshape(-1,3).detach().cpu().numpy()
        pc2_gt = torch.squeeze(pc2_gt).reshape(-1,3).detach().cpu().numpy()
        #print("pc1_range_0", np.unique(pc1[:, 0]))
        #print("pc1_range_1", np.unique(pc1[:, 1]))
        #print("pc1_range_2", np.unique(pc1[:, 2]))
        #print(stop)
        if pc1 is None:
            return None, None, None, None, None
        ...
        #if self.DEPTH_THRESHOLD > 0:
        near_mask_depth = np.logical_and(pc1[:, 2] < self.DEPTH_THRESHOLD, pc2[:, 2] < self.DEPTH_THRESHOLD)
        #near_mask_height_oppo = np.logical_and(pc1[:, 1] < self.HEIGHT_THRESHOLD, pc2[:, 1] < self.HEIGHT_THRESHOLD)
        #near_mask_height = np.logical_not(near_mask_height_oppo)
        #near_mask = np.logical_and(near_mask_depth, near_mask_height)
        near_mask = near_mask_depth
        '''
        ...
        pcl_mask = torch.flatten(torch.squeeze(torch.squeeze(pcl_mask_gt) ,0))
        mask0 = (pcl_mask==1).nonzero().view(-1).detach().cpu().numpy()
        if len(mask0) < self.num_points:
            print("less than 8192 points after mask")
            return None, None, None, None, None
        indices = np.arange(len(mask0))
        mask1 = np.sort(np.random.choice(indices, size=self.num_points, replace=False, p=None))
        if self.no_corr:
        ...
        print("maskall_1", maskall_1.shape)
        print("pc1_gt", pc1_gt.shape)
        print("pc2_gt", pc2_gt.shape)
        print(stop)'''
        return pc1, pc2, maskall_1, maskall_2, sf_gt, pc1_gt, pc2_gt
