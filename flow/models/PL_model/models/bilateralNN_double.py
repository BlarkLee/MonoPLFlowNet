...
class BilateralConvFlex(nn.Module):
    ...
    def forward(self, features,
                ...
                out_barycentric_upsample=None, out_lattice_offset_upsample=None):
        """
        ...
        #print("in_barycentric", in_barycentric.shape)
        #print("in_lattice_offset", in_lattice_offset.shape)
        #print("blur_neighbors", blur_neighbors.shape)
        #print("out_barycentric", blur_neighbors.shape)
        #print("out_lattice_offset", out_lattice_offset.shape)
        # -------------------- SLICE --------------------
        ...
        #print("tmp_feat_blurred", temp_feat_blurred.shape)
        ...
        if out_lattice_offset_upsample is not None:
            tmp_feat_blurred_upsample = feat_blurred[batch_indices[:, None, None, None],
                                        self.out_indices[None, :, None, None],
                                        out_lattice_offset_upsample[:, None, :, :]]
            sliced_upsample = (out_barycentric_upsample[:, None, :, :] * tmp_feat_blurred_upsample).sum(dim=2)
            if self.use_bias:
                sliced_upsample += self.bias[None, :, None]
                
            return sliced, sliced_upsample
