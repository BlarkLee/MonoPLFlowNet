...
from .interpolation_multi import interpolate2d_as, my_grid_sample, get_grid, upsample_flow_as
...
class FeatureProp_Softsplat(nn.Module):
    ...
    def forward(self, x, flow, disp, feat1, feat2):
        ...
        disocc = FunctionSoftsplat(tenInput=mask, tenFlow=flow, tenMetric=None, strType='summation')
        disocc_map = (disocc > 0.5).to(dtype=flow.dtype)

        if disocc_map.sum() < (b * h * w / 2):
            return torch.zeros_like(x)
        else:
            x_warped = FunctionSoftsplat(tenInput=x, tenFlow=flow, tenMetric=-20.0 * (0.4-disp), strType='softmax')
            feat1_warped = FunctionSoftsplat(tenInput=feat1, tenFlow=flow, tenMetric=-20.0 * (0.4-disp), strType='softmax')
