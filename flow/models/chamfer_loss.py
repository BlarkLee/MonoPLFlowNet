...
def chamfer_loss(pc1, pc2, flow):
    pc1_warp = pc1 + flow
    dist1, dist2 = computeChamfer(pc1_warp, pc2)
    return dist1.sum(dim = 1).mean() + dist2.sum(dim = 1).mean() #dist1.mean()+dist2.mean() #dist1.sum(dim = 1).mean() + dist2.sum(dim = 1).mean()